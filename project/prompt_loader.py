...
def load_energy_diagnosis_prompts():
    try:
        diagnosis_prompt_path=get_abs_path(prompts_conf["energy_diagnosis_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_energy_diagnosis_prompts]在yaml配置项中没有energy_diagnosis_prompt_path配置项")
        raise e

    try:
        return open(diagnosis_prompt_path,"r",encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_energy_diagnosis_prompts]解析能耗诊断提示词出错,{str(e)}")
        raise e
